...
def get_goal_color(goal, value, mom_value=None):
    ...
    if mom_value:
        previous_outcome = mom_value
    else:
        previous_outcome = goal.previous_outcome

    if goal.goal_type.upper() == "ABOVE":
        if value < goal.target or value < previous_outcome:
            return DANGER_COLOR
        elif value >= goal.target and value >= previous_outcome:
